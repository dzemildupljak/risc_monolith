...
const donorByBoodType = `-- name: DonorByBoodType :one
SELECT donor_id, donor_unique_identification_number, donor_name, donor_surname, donor_address, donor_last_donation, donor_phone_number, donor_blood_type, donor_blood_type_num FROM donors
WHERE donor_blood_type = $1
LIMIT 1
`

func (q *DonorRepository) DonorByBoodType(ctx context.Context, donorBloodType string) (domain.Donor, error) {
	row := q.Queries.db.QueryRowContext(ctx, donorByBoodType, donorBloodType)
	var i domain.Donor
	err := row.Scan(
		&i.DonorID,
		&i.DonorUniqueIdentificationNumber,
...
func (q *DonorRepository) DonorsByBloodTypeNum(ctx context.Context, donorBloodTypeNum int16) ([]domain.Donor, error) {
